...
/**
 * Shared NextAuth config. Kept outside the route handler so API routes can
 * import it without pulling the App Router handler into Turbopack HMR graphs
 * (which can briefly 404 /api/auth/session and trigger CLIENT_FETCH_ERROR).
 */
        ...
        async jwt({ token, user, trigger, session, account }) {
            if (user) {
                if (account?.provider === "google") {
                    ...
                    token.provider =
                        (user as { provider?: string }).provider || "email";
                }
            }
